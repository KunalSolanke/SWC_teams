...
dockerimages = {
  postgres: async (project) => {
    ...
    await utils.spawnCommand(
      commands["createVolume"](volumeName),
      "create pg vol",
      utils.cb
    );
    ...
    let database = await new Database({
      project: project._id,
      user: project.user,
      contianername: contianername,
      name: "postgres",
      port: port,
      type: "SQL",
    });

    database.configs = database.configs.concat([
      { key: "DATABASE_URI", value: `${container_name}:${port}` },
    ...
    project.configs = project.configs.concat([
      { key: "DATABASE_URI", value: `${container_name}:${port}` },
      { key: "POSTGRES_USER", value: `${project.user.username}` },
      { key: "POSTGRES_HOST", value: `${contianername}` },
      { key: "POSTGRES_PASSWORD", value: `${contianername}` },
  ...
  mongo: async (project) => {
    ...
    try {
      ...
      await utils.spawnCommand(
        commands["createVolume"](volumeConfig),
        "create mongo config vol ",
        utils.cb,
        fallbackArr
      );
      await utils.spawnCommand(
        commands["dockerRun"](
          `-dit --rm --name ${containername} --net nginx-proxy -p ${port}:27017 mongo`,
          containername
        ),
        `mongo container ${project.name}`,
        utils.cb,
        fallbackArr
      );
    } catch (err) {
    ...
    database.configs.push({
      key: "MONGO_URI",
      value: `mongodb://${containername}:27017/${project.name}`,
    });
    database;

    project.databases.push(database._id);
    project.config_vars.push({
      key: "MONGO_URI",
      value: `mongodb://${containername}:27017/${project.name}`,
    });
    project.save();
